...
const uploadCover = ({ file, token, observer }) => {
  // 判断图片格式
  if (!imgSuffix[file.type]) {
    Message({
      showClose: true,
  ...
  // 构造存储路径
  let suffix = file.name.slice(file.name.lastIndexOf("."))
  let key = imageDir + "/" + uuid() + suffix
  // 上传文件
  upload({ file, key, token, observer })
}

const uploadImg = ({ file, token, observer }) => {
  // 构造存储路径
  let suffix = file.name.slice(file.name.lastIndexOf("."))
  let key = imageDir + "/" + uuid() + suffix
  // 上传文件
...
const uploadArticle = ({ file, token, observer, key = null }) => {
  if (key) {
    // 如key存在，则认定为覆盖上传
    upload({ file, key, token, observer })
  } else {
    // 构造随机存储路径
    let suffix = file.name.slice(file.name.lastIndexOf("."))
    let key = articleDir + "/" + uuid() + suffix
    // 上传文件
    upload({ file, key, token, observer })
  }
}
